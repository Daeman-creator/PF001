...
var fncPageBoard = function(){
	...
	if(gubun != "pop"){
		if ( $("#kdnDept1").length > 0 ) { $("#kdnDept1").prop('disabled',false); }
		if ( $("#kdnDept2").length > 0 ) { $("#kdnDept2").prop('disabled',false); }
	}
	switch (gubun) {
	  ...
	  case 'view'   :  if(seqNm.length > 0 && seqVal.length > 0){ for (var i = 0; i < seqNm.length; i++) { $("#"+seqNm[i]).val('');$("#"+seqNm[i]).val(seqVal[i]); } }
					   $("#defaultFrm").attr({"action" : url, "method" : "get", "target" : "_self", "onsubmit" : ""}).submit();//상세
		  			   break;
	  ...
	  case 'submit' :  if(process == "Y"){ alert("처리중입니다. 잠시만 기다려주세요."); return false;}//등록 액션
					  // if(wrestSubmit(document.defaultFrm)){ 
					   ...
					   //}
					   break;
	  case 'pop'   :  console.log(het);window.open('', url.replace(".do",""), 'top=120,left=50,titlebar=no,status=no,toolbar=no,resizable=no,scrollbars=yes,width='+wth+'px, height='+het+'px');
	  				  if(seqNm.length > 0 && seqVal.length > 0){ for (var i = 0; i < seqNm.length; i++) { $("#"+seqNm[i]).val('');$("#"+seqNm[i]).val(seqVal[i]); } }
					  $("#defaultFrm").attr({"action" : url, "method" : "post", "target" : url.replace(".do","")}).submit();
	  break;
